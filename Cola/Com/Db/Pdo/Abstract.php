...
abstract class Cola_Com_Db_Pdo_Abstract extends Cola_Com_Db_Abstract
{
    ...
    public function bindSql($sql, $array, $typeArray = false)
    {
        ...
        if (is_object($this->_query) && ($this->_query instanceof PDOStatement)) {
            foreach ($array as $key => $value) {
                if ($typeArray) $this->_query->bindValue(":$key", $value, $typeArray[$key]);
                else {
                    ...
                    if ($param) $this->_query->bindValue(":$key", $value, $param);
                }
            }
