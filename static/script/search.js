jQuery(function ($) {
    ...
    // 公共搜索下拉菜单独有事件绑定
    var publicSearch = $('.m-schBox')
    publicSearch.find('li.dropdownCell').bind({
        click: function (e) {
            var url = $(this).find('a').attr('href') + '/search';
            publicSearch.find('form').attr('action', url);
        }
    }).eq(0).trigger('click');
    //初始化下拉列表
    $('[name="topDropdown"]').topDropdownSelect('customDropdown1MouseOver');
    ...
    initSearch: function () {
        ...
        main.extend({
            ...
            showsuggest: function (keyword) {
                var url = main.parent().attr('action').replace('/search','');
                jQuery.getJSON(url+'/suggest/key/' + keyword, '', function (data) {
                    if (data.length > 0) {
                        var html = '';
            ...
            search: function (keyword) {
                if (keyword.length > 1) {
                    ...
                    window.location.href = url+'?key=' +keyword;
                }
            }
        }).bind({
            keyup: function (e) {
