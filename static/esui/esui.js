/**
 * jQuery EasyUI 1.3.5
 * 修正By：sasou admin@sy-y.com
 * 1、提示框定时关闭
 * 2、datagrid、treegrid增加排序属性
 * 3、combotree 增加禁止选择属性
 * 4、combotree 增加onAutoLoadSuccess事件
 */
(function($){
$.parser={auto:true,onComplete:function(_1){
},plugins:["draggable","droppable","resizable","pagination","tooltip","menu","menubutton","splitbutton","progressbar","tree","combobox","combotree","combogrid","numberbox","validatebox","searchbox","numberspinner","timespinner","calendar","datebox","datetimebox","slider","layout","panel","datagrid","propertygrid","treegrid","tabs","accordion","window","dialog","linkbutton"],parse:function(_2){
var aa=[];
...
$.fn.pagination.defaults={total:1,pageSize:10,pageNumber:1,pageList:[10,20,30,50,100,200],loading:false,buttons:null,showPageList:true,showRefresh:true,links:10,layout:["list","sep","first","prev","sep","manual","sep","next","last","sep","refresh"],onSelectPage:function(_a6,_a7){
...
if(!opts.treeauto){
opts.onAutoLoadSuccess.call(_ff,_103,data);
}
};
...
var t=$(_192);
return $.extend({},$.parser.parseOptions(_192,["url","treeauto","method",{checkbox:"boolean",cascadeCheck:"boolean",onlyLeafCheck:"boolean"},{animate:"boolean",lines:"boolean",dnd:"boolean"}]));
};
...
var is_checkbox = (typeof(item.checked)=="undefined")?"":item.checked;
...
$.fn.tree.defaults={url:null,method:"post",animate:false,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,lines:false,treeauto:false,dnd:false,data:null,formatter:function(node){
return node.text;
...
},onAutoLoadSuccess:function(node,data){//treeauto
...
var opts=(typeof($.data(_1fe,"panel").options)!='undefined')?$.data(_1fe,"panel").options:null;
...
return win;
},msg:function(_277,msg,icon,fn,time){
var _278="<div>"+msg+"</div>";
switch(icon){
case "error":
_278="<div class=\"messager-icon messager-error\"></div>"+_278;
...
_279[$.messager.defaults.ok]=function(){
if(fn){
$("body").prepend('<script>'+fn+';</script>');
}
if(win){win.window("close");win = null;}
return false;
};
var win=_271(_277,_278,_279);
if (time>0) {
...
var trindex = (typeof(rows[i]['trindex'])!='undefined')?rows[i]['trindex']:'';
var trid = (typeof(rows[i]['trid'])!='undefined')?rows[i]['trid']:'';
var level = (typeof(rows[i]['level'])!='undefined')?rows[i]['level']:'';
_68e.push("<tr id=\""+_692+"\" datagrid-row-index=\""+i+"\" trindex=\"" +trindex+"\" trid=\""+trid +"\" level=\"" +level+"\" "+cls+" "+_691+">");
...
var trindex = (typeof(row['trindex'])!='undefined')?row['trindex']:'';
var trid = (typeof(row['trid'])!='undefined')?row['trid']:'';
var level = (typeof(row['level'])!='undefined')?row['level']:'';
_7bc.push("<tr id=\""+_7c0+"\" node-id=\""+row[opts.idField]+"\" trindex=\"" +trindex+"\" trid=\""+trid +"\" level=\"" +level+"\" "+cls+" "+_7bf+">");
...
tree.tree({url:opts.url,treeauto:opts.treeauto});
});
