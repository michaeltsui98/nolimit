 ...
 */
class Modules_Sm_Controllers_Lesson extends Modules_Sm_Controllers_Base
{
    ...
    public function recordAction() {
        ...
        $start = max(0, ($page-1)*$limit);
        $extend_conditions = $this->getFolderConditions();
        //var_dump($extend_conditions);die;
    ...
    public function chapterAction() {
        ...
        if ($grade) {
            $this->view->grade = $grade;
            $this->view->topNodeList = Models_Node::getTopNodeList($this->user_info['xd'], $this->view->xk_code, $edition, $grade);
        }
    ...
    public function addAction() {
        ...
        if (Cola_Request::isPost()) {
            ...
            if ($message) {
                ...
                $default_title = '';
                // 设置备课夹的默认标题
                $node_info = Models_Resource::init()->getUnitTitleById($post_data['node']);
                if ($node_info) {
                    $default_title = $node_info['data'].'【备课】';
                }
    ...
    public function addResourceAction() {
        ...
        if ($folder_id) {
            ...
            if ($folder_info) {
                ...
                if ($favResourceList) {
                    $favResourceList = array_map(array($resource_model, 'getResourceUserInfo'), $favResourceList['rows']);
                    $this->view->favResourceList = $favResourceList;
                }
