 ...
 * @author libo
 */
class Modules_Xl_Controllers_Study extends Modules_Xl_Controllers_Base
{
    ...
    public function __construct()
    {
        ...
        // $this->_grade_code = $this->_config['nj_code'][$this->_grade];
    ...
    public function studyDetailMoreAjaxAction()
    {
        ...
        $node = $this->study_config->build_tree($info); //知识树
    ...
    public function studyHistoryMoreAjaxByNodeAction()
    {
        ...
        $page_config = $this->study_config->pageConfig();
        //知识单元节点
        $info = Models_Resource::init()->getUnit(empty($this->user_info['xd']) ? $this->_defalut_xd : $this->user_info['xd'], $this->view->xk_code, $default_version, $grade); // 'GO003-1'
    ...
    public function getDifferentResourceTypeAction()
    {
        ...
        if (!empty($info)) {
            foreach ($info as $key => $v) {
                if ($study_type_info['type_id'] != 100) {// 100:测评作业,5 :微视频
                    $data[$key]['title'] = $v['study_resource_title'];
                    $data[$key]['url'] = '/' . $this->view->xk . '/Study/learin/' . $v['study_resource_id'];
                    $data[$key]['size'] = ($study_type_info['type_id'] == 5) ? $this->study_config->time($v['study_resource_size']) : 0;
                } else {
                    $data[$key]['title'] = $v['study_resource_title'];
