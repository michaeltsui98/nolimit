 ...
 * @author libo
 */
class Modules_Xl_Controllers_Evaluate extends Modules_Xl_Controllers_Base
{
    ...
    public function compositeEvaluateAction()
    {
        ...
        $nj = $this->post('nj');
        $bb = $this->post('bb');
        $evaluate_type = $this->post("evaluate_type");
        $page_config = $this->study_config->pageConfig();
        $data = Models_Evaluate_Evaluate::init()->getUserEvaluateListByClassify($this->view->xk_code, $evaluate_type, $this->user_info['xd'], $nj, $bb, $page_config['start'], $page_config['limit']);
    ...
    {
        $config = $this->study_config->config();
        $nj = $this->post('nj'); //单元测评时,我要查询当前用户某个年级所在的班级
        $bb = $this->post('bb');
        //获取班级id
    ...
    public function doEvaluateAction()
    {
        ...
        if ($type == $config['_evaluate_type']['study_evaluate']) {
            ...
            $param = $evaluate_id . "_" . $config['_evaluate_type']['study_evaluate'] . '_' . $id;
            $_SESSION['evaluate_param'] = $param;
            $url = HTTP_XUE . 'test.htm?id=' . $evaluate_id;
        } else {
            //根据主键id获取相关信息
            $info = Models_Evaluate_Evaluate::init()->getEvaluateById($id);
            if (empty($info['evaluate_type'])) {
                //内部应用
                if ($evaluate_classify == 1) {
                    //综合测评
                    $this->comEvaluateStatusAction($id, $evaluate_id, $evaluate_classify);
                } else if ($evaluate_classify == 2) {
                    //单元测评
                    $this->unitEvaluateStatusAction($id, $evaluate_id, $evaluate_classify, $evaluate_course_id);
                }
                //从学习记录表里面获取学习的状态
                ...
                $param = $evaluate_id . "_" . $config['_evaluate_type']['evaluate'] . "_" . $evaluate_classify . "_" . $id . "_" . $evaluate_course_id;
                $_SESSION['evaluate_param'] = $param;
                $url = HTTP_XUE . 'test.htm?id=' . $evaluate_id;
            } else {
                //和易小站的测评,用户信息的封装
                $user_url = $this->heyiDataAction();
                //综合和趣味心理测评
                $this->comEvaluateStatusAction($id, $evaluate_id, $evaluate_classify);
                //从学习记录表里面获取学习的状态
                $record_info = Models_Evaluate_EvaluateRecord::init()->getCurEvaluateInfo($this->user_info['user_id'], $id, $evaluate_course_id);
                ...
                $id_course_id = $id . '_' . $evaluate_course_id;
                $url = "http://heyi.dodoedu.com/remote/test/question_init.do?questionId=" . $evaluate_id . '&ids=' . $id_course_id . "&" . $user_url;
            }
        }
        // echo  $url;
     ...
     * 综合测评的状态
     * @param   int $id 主键id
     * @param   int $evaluate_id 测评试卷id
     * @param   int  $evaluate_classify  分类id
     * @return int
     */
    public function comEvaluateStatusAction($id, $evaluate_id, $evaluate_classify)
    {
        //综合测评
        $info = Models_Evaluate_Evaluate::init()->getPartFieldsById($id, 'evaluate_end_time');
        if (empty($info[0]['evaluate_end_time'])) {
            $evaluate_record_status = 1;
        } else {
     ...
     * @return array
     */
    public function unitEvaluateStatusAction($id, $evaluate_id, $evaluate_classify, $evaluate_course_id)
    {
        //单元测评
     ...
     */
    public function heyiDataAction()
    {
        $age = empty($this->user_info['user_birthday']) ? 0 : (date("Y", time()) - substr($this->user_info['user_birthday'], 0, 4));
        $users = Models_StudyResource_StudyRecordConfig::init()->getCurClassSchoolByUserId();
        $gradeName = isset($users['class_id']) ? urlencode(Models_StudyResource_StudyRecordConfig::init()->getGrade($users['xd'], $users['grade_id'])) : '';
        $user = array(
            'userId' => $this->user_info['user_id'],
            'userName' => urlencode($this->user_info['user_name']),
            'realName' => urlencode($this->user_info['user_realname']),
            'sex' => $this->user_info['user_sex'],
            'icon' => $this->user_info['user_avatar_64'],
            'age' => $age,
            'classId' => isset($users['class_id']) ? $users['class_id'] : 0,
            'className' => isset($users['class_id']) ? urlencode($users['class_name']) : '',
            'gradeId' => isset($users['class_id']) ? $users['grade_id'] : 0,
            'gradeName' => $gradeName,
            'schoolName' => urlencode($this->user_info['school_name']),
            'schoolId' => $this->user_info['school_id'],
            'birthDay' => $this->user_info['user_birthday']
        );
        $user_url = http_build_query($user);
        return $user_url;
    }
    ...
    public function evaluateHistoryAjaxAction()
    {
        ...
        switch ($evaluate_type) {
            ...
            case 'unit_evaluate':
                ...
                break;
            case 'interest_evaluate':
                $evaluate_classify = $config['_evaluate_classify']['interest'];
                break;
        }
