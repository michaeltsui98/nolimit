 ...
 * @author libo
 */
 class Modules_Xl_Controllers_Parent extends Modules_Xl_Controllers_Base
 {
    ...
    protected $_defalut_xd = 'xd001';
 	...
 	public function __Construct()
 	{
        ...
        $this->_course = new Modules_Xl_Controllers_Course();
 ...
 public function ajaxAddMyAppAction()
 {
    ...
    */
    ...
    public function studyHistoryCountAction()
    {
         ...
         //$xds = empty($this->study_config->getCurClassSchoolByUserId('xd',$child_id));
         //$xd = empty($xds)?$this->_defalut_xd:$xds;
   ...
   public function courseAddQuestionForCourseAction()
   {
    ...
    public function studyHistoryMoreAjaxByNodeAction()
    {
        ...
        //知识单元节点
         $info = Models_Resource::init()->getUnit(empty($child_info['xd_code']) ?$this->_defalut_xd : $child_info['xd_code'], $this->view->xk_code, $default_version, $grade_node);
        //$info = Models_Resource::init()->getUnit('xd003', $this->view->xk_code, 'v11', 'GO0027');  
        ...
        if (!empty($node)) {
            foreach ($node as $key => $value) {
                if (isset($value['child'])) {
                    ...
                    $info = $this->_study_record->getStudyResourceCount($child_info['child_id'], $this->view->xk_code, empty($child_info['xd_code']) ?$this->_defalut_xd : $child_info['xd_code'], $default_version, $grade_node, $value['id']);
  ...
  {
    // if($this->user_info['user_name'] == 'bookg02'){
    //     echo $xd_code;
    //     print_r(Models_Resource::init()->getBbByXdAndXk($xd_code, $this->view->xk_code));
    // }
